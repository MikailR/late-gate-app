...
/**
 * Installs MiniKit with the portal app id so `MiniKit.isInstalled()` is true
 * inside World App. In a normal browser the install reports failure and the
 * app keeps running on the mock adapters.
 */
export function WorldMiniKitProvider({ children }: { children: ReactNode }) {
  return <MiniKitProvider props={{ appId: railsEnv.worldAppId || undefined }}>{children}</MiniKitProvider>;
}
...
export function useWorldAppStatus(): WorldAppStatus {
  ...
  return { installed: isInstalled, appId: railsEnv.worldAppId };
}

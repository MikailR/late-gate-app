...
The app installs MiniKit on load (`NEXT_PUBLIC_MINIKIT_APP_ID`, the Late Gate Mini App; IDKit uses the
separate World ID app id `NEXT_PUBLIC_WORLD_APP_ID`), so it runs inside World App as the
Mini App **Late Gate** as well as in a phone browser. Inside World App the wallet and World ID paths use
